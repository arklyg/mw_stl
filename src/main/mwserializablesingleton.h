...
class MWSerializableSingleton {
  protected:
    static std::vector<MWSerializableSingleton*> _instance_pt_vector;

    size_t _class_id;

  public:
    static MWSerializableSingleton *GetSerializableSingletonInstance(
        size_t class_id);

    virtual ~MWSerializableSingleton();

    virtual void SetClassID(size_t class_id);
